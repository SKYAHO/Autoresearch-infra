...
- Kubernetes admin root는 `terraform/admin/` 하위에서 별도 state로 관리합니다:
  ...
  `autoresearch-k8s`(앱 namespace/KSA 경계 — `experiment_jobs.tf`가 Job 종류별
  어드미션 계약·quota·Phase 1 egress를, `experiment_executor.tf`가 #562 Phase 2
  executor egress를 소유), `actions-runner-k8s`(#533 셀프
