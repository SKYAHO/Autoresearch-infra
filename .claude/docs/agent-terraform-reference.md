...
- `<리소스종류>.tf` — `vpc.tf`, `nat.tf`, `cloud_sql.tf`, `gke.tf`,
  `artifact_registry.tf`, `secret_manager.tf` 처럼 종류별 분리
...
- GCS remote backend 도입(#6) 전까지 state는 로컬 관리입니다. state를
  다루는 작업(mv, rm, import)은 사용자 확인 후 진행합니다.
