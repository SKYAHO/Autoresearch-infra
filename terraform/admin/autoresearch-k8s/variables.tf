...
variable "agent_orchestration_pull_request_k8s_service_account" {
  description = "실험 PR 생성기의 전용 Kubernetes service account 이름."
  type        = string
...
variable "agent_orchestration_pull_request_gcp_service_account_email" {
  description = "실험 PR 생성기 GSA email. 빈 값이면 resource_prefix/project_id에서 dev 기본값을 파생한다."
  type        = string
  default     = ""

  validation {
    condition = (
      trimspace(var.agent_orchestration_pull_request_gcp_service_account_email) == "" ||
      can(regex("^[^@[:space:]]+@[^@[:space:]]+\\.iam\\.gserviceaccount\\.com$", var.agent_orchestration_pull_request_gcp_service_account_email))
    )
    error_message = "agent_orchestration_pull_request_gcp_service_account_email must be a GSA email when set."
  }
}
