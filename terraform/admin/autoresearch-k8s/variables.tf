...
variable "private_services_cidr" {
  description = "Private Service Access CIDR containing Cloud SQL and Redis endpoints."
  type        = string
...
variable "redis_port" {
  description = "Memorystore TLS endpoint port from terraform/envs/dev redis_port output."
  type        = number
  default     = 6378

  validation {
    condition     = var.redis_port >= 1 && var.redis_port <= 65535 && floor(var.redis_port) == var.redis_port
    error_message = "redis_port must be an integer between 1 and 65535."
  }
}
