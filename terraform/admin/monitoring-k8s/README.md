...
실제 `kube-prometheus-stack` Helm release 설치는 Issue #79에서 진행합니다.
...
- `monitoring` namespace를 만들 수 있는 Kubernetes 권한
...
`helm-values/kube-prometheus-stack.values.yaml`은 #79에서 Helm 설치에 사용할
기본 values 위치입니다. 이 파일에는 secret 값을 넣지 않습니다.
...
Private GKE cluster에서 admission webhook 접근 문제가 발생하면 #79에서 firewall
또는 chart option 조정을 검토합니다.
