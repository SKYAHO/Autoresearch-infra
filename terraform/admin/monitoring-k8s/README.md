...
- Grafana port-forward 전용 최소 RBAC
...
| Grafana access RBAC | `grafana-port-forward` Role | port-forward에 필요한 최소 권한 |
...
`grafana_viewer_user_emails`에 Google 계정을 넣고 apply합니다. 이 권한은
`monitoring` namespace에서 Grafana port-forward에 필요한 read/portforward 권한만
부여하며, cluster-admin 권한이 아닙니다.
