...
검증된 `Autoresearch` release의 Agent Orchestration API·UI immutable digest를
...
- `Autoresearch` release workflow는 OCI revision·non-root 검증을 통과한 API·UI
...
1. `Autoresearch` release workflow에서 API·UI build, OCI revision, non-root,
   infra promotion Job이 모두 성공했는지 확인합니다.
2. infra `main` commit에서 API 일곱 참조와 UI 한 참조가 같은 release의 immutable
   digest로만 바뀌었는지 확인합니다.
...
- 잘못된 digest가 배포된 경우에는 이전에 검증된 API·UI digest를 release
