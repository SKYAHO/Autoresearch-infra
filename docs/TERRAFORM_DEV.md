...
`autoresearch-dev-airflow-deployer` GSA를 WIF로 가장할 수 있다. GSA에는 GKE DNS
...
| `SKYAHO/Autoresearch` | `autoresearch-dev-app-pusher` | 정확한 release workflow + source ref(`workflow_ref`) principalSet만 허용 | `roles/artifactregistry.writer` on `autoresearch-dev-docker` |
...
`repository_ref`(repo@ref), application pusher SA는
`workflow_ref`(repo/workflow@workflow-source-ref)로 제한해 임의 브랜치의
workflow가 image push SA를 가장하는 것을 차단한다. release event의 `ref`는
tag이므로 application 경계에는 `repository_ref`를 사용하지 않는다.
