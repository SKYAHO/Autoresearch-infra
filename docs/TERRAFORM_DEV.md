...
|---|---|---|---|
| `SKYAHO/Autoresearch-airflow` | `autoresearch-dev-gar-pusher` | 해당 저장소 principalSet만 허용 | `roles/artifactregistry.writer` on `autoresearch-dev-docker` |
| `SKYAHO/Autoresearch` | `autoresearch-dev-app-pusher` | 해당 저장소 principalSet만 허용 | `roles/artifactregistry.writer` on `autoresearch-dev-docker` |
...
provider 허용 목록과 SA별 principalSet을 모두 통과해야 하며, `terraform-ci` SA
