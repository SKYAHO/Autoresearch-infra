...
| `SKYAHO/Autoresearch` | `autoresearch-dev-app-pusher` | 저장소 **+ 승인 ref**(`repository_ref`) principalSet만 허용(#175) | `roles/artifactregistry.writer` on `autoresearch-dev-docker` |
...
provider 허용 목록과 SA별 principalSet을 모두 통과해야 한다. pusher SA는
`repository` 단독이 아니라 `repository_ref`(repo@ref) 조합으로 제한해 **임의
브랜치의 workflow가 이미지 push SA를 가장하는 것을 차단한다**(#175, 승인
ref 기본값 `refs/heads/main` — 앱 저장소 배포 방식 확정 후 조정). `terraform-ci` SA
가장은 여전히 infra 리포만 가능하다(2단 경계).
...
적용 순서는 bootstrap provider 허용 목록 갱신 후 dev root SA/IAM 생성이다.
