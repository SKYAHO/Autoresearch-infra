...
| Cloud Build bucket 권한 | `roles/storage.objectAdmin` + `roles/storage.legacyBucketReader` on `ar-infra-501607_cloudbuild` | source 읽기와 로그 쓰기. `objectAdmin`에는 `storage.buckets.get`이 없어 legacyBucketReader가 함께 필요하다(#204 교훈) |
| Logging 권한 | `roles/logging.logWriter` on project | build log 기록 |
...
gcloud builds submit \
  ...
  --gcs-log-dir=gs://ar-infra-501607_cloudbuild/logs \
  --tag=asia-northeast3-docker.pkg.dev/ar-infra-501607/autoresearch-dev-docker/<image>:<tag> .
```

> 사용자 지정 SA로 실행하는 build는 로그 위치를 명시해야 한다(`--gcs-log-dir` 없이
> 실행하면 기본 로그 버킷 정책과 충돌한다). 검증: 2026-07-20 smoke build로 위 명령의
> push 성공을 확인했다(테스트 이미지는 삭제).
