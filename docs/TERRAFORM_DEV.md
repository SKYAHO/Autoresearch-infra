...
- **두 apply workflow의 공용 구현(#448)**: `admin-apply.yml`·`dev-apply.yml`은 진입점만 남고, plan→승인→apply 파이프라인(요약 게시·이메일 마스킹·plan 바이너리 GCS 전달·orphan plan 정리)은 `.github/workflows/terraform-apply.yml`(`workflow_call`) 한 곳에 있다. 두 진입점이 유지되는 이유는 보안 경계다 — GitHub OIDC의 `workflow_ref` claim은 **최상위 호출 워크플로우**를 가리키므로(재사용 워크플로우 자신은 `job_workflow_ref`), apply SA 2종의 WIF 가장 조건(`admin-apply.yml@main`/`dev-apply.yml@main`)과 SA 분리, Environment·concurrency 분리가 리팩터 후에도 그대로 유지된다. 즉 이 변경에 Terraform/IAM 변경은 없다. target별 차이는 호출부 입력으로만 표현한다: `roots`(apply 순서 포함), `plan_prefix`, `environment`, `plan_service_account`(dev만 읽기 전용 CI SA), `apply_service_account`, `guard_admin_allowlist`(admin만 true). plan 경로는 두 target 모두 `<target>-apply-plans/<root>/<run_id>.tfplan`으로 통일됐다(dev root의 `<root>`는 `dev`).
