...
- `terraform/admin/airflow-k8s`: Kubernetes namespace/RBAC/ResourceQuota/LimitRange/NetworkPolicy만 관리한다. GKE API 서버 접근이 필요해 GitHub Actions PR plan이 이 root를 실행하지 않는다. apply는 DNS 엔드포인트(`--dns-endpoint`, IAM 검증) kubeconfig로 수행한다. #279로 `master_authorized_networks`는 비어 있어 IP 엔드포인트 경로는 쓰지 않으며, K8s admin root는 `admin-apply.yml` 승인 게이트 CI apply(#307/#312)로도 적용한다.
...
- **두 apply workflow의 공용 구현(#448)**: `admin-apply.yml`·`dev-apply.yml`은 진입점만 남고, plan→승인→apply 파이프라인(요약 게시·이메일 마스킹·plan 바이너리 GCS 전달·orphan plan 정리)은 `.github/workflows/terraform-apply.yml`(`workflow_call`) 한 곳에 있다. 두 진입점이 유지되는 이유는 보안 경계다 — GitHub OIDC의 `workflow_ref` claim은 **최상위 호출 워크플로우**를 가리키므로(재사용 워크플로우 자신은 `job_workflow_ref`), apply SA 2종의 WIF 가장 조건(`admin-apply.yml@main`/`dev-apply.yml@main`)과 SA 분리, Environment·concurrency 분리가 리팩터 후에도 그대로 유지된다. 즉 이 변경에 Terraform/IAM 변경은 없다. target별 차이는 호출부 입력으로만 표현한다: `roots`(apply 순서 포함), `plan_prefix`, `environment`, `plan_service_account`(dev만 읽기 전용 CI SA), `apply_service_account`, `guard_admin_allowlist`(admin만 true). secret은 `secrets: inherit`(repo 전체 전달) 대신 `workflow_call.secrets` 명시 선언으로 넘긴다 — admin 호출부만 allowlist 6종을 매핑하고 dev 호출부는 하나도 넘기지 않아, 최강 SA로 인증되는 dev apply job의 secret 노출면이 리팩터 전과 동일하게 유지된다. plan 경로는 두 target 모두 `<target>-apply-plans/<root>/<run_id>.tfplan`으로 통일됐다(dev root의 `<root>`는 `dev`). **plan 회수 절차**(승인 전 상세 diff 확인용): `gcloud storage cp gs://autoresearch-503903-dev-tfstate/<target>-apply-plans/<root>/<run_id>.tfplan /tmp/p.tfplan` → 해당 root 디렉터리에서 **CI와 같은 버전**(1.13.5)의 terraform으로 `terraform show -no-color /tmp/p.tfplan` (버전이 다르면 plan 파일을 읽지 못한다). **왕복 검증(dispatch → plan 성공 확인 → `gh run cancel`) 후 정리**: 취소 run은 apply job의 cleanup이 돌지 않아 plan 객체가 남는다(dev 1건 = `dev-apply-plans/dev/<run_id>.tfplan`, admin 7건 = `admin-apply-plans/<root>/<run_id>.tfplan`). 다음 dispatch 시 `Cleanup stale plans`가 어차피 지우지만, plan 바이너리는 민감 속성값을 포함할 수 있으므로 노출 창을 좁히려면 검증 직후 수동 삭제한다 — 실행 주체는 CI SA가 아니라 **state 버킷 objectAdmin을 가진 운영자 계정**(로컬 `gcloud`)이다. **주의**: 같은 prefix를 쓰는 다른 run이 승인 대기 중이면 prefix 와일드카드(`.../<target>-apply-plans/**`)로 지우지 말고 자기 `<run_id>`만 지정해 삭제한다(공용 구현 상단 불변식 ①과 같은 취지 — 남의 plan을 지우면 그 run은 승인 후 `gcloud storage cp`에서 실패한다). 호출부를 추가할 때 지켜야 할 불변식 2가지는 `terraform-apply.yml` 상단 주석에 있다: ① `plan_prefix` ↔ `concurrency.group` 1:1(공유 시 승인 대기 run의 plan이 지워짐) ② target별 root 변수 이름공간 비충돌(겹치면 TF_VAR 합집합이 조용히 주입됨).
