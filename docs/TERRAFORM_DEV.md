...
- **admin root gated apply(#307)**: `.github/workflows/admin-apply.yml`이 `workflow_dispatch`(입력 `root`)로 admin root를 apply한다. plan job 요약 → `admin-apply` Environment reviewer 승인 → apply job. 전용 apply SA(`autoresearch-dev-admin-apply`, WIF `admin-apply.yml@main` 제한, `roles/container.admin`)를 쓴다. 민감 tfvars(허용 이메일)는 로컬 파일이 아니라 GitHub Secrets 단일 원천에서 와 팀원 간 tfvars 불일치 사고(#305)를 막는다. OAuth client secret 등 K8s Secret payload는 여전히 operator 주입(CI 밖). 로컬 apply는 break-glass. 현재 파일럿 대상은 `argocd-k8s`.

필요 GitHub variables: `GCP_PROJECT_ID`, `WIF_POOL_ID`, `WIF_PROVIDER_ID`, `CI_SA_EMAIL`(plan/drift), `ADMIN_APPLY_SA_EMAIL`(#307). Secrets: `ARGOCD_ADMIN_USER_EMAILS`(#307, JSON 리스트). Environment: `admin-apply`+required reviewers(#307).
