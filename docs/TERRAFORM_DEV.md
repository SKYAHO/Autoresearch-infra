...
│       ├── github_actions.tf # #121 Autoresearch-airflow GitHub Actions WIF → GAR push SA/IAM
...
release를, #85에서 AppProject(`autoresearch-dev`)와 샘플
Application(`sample-guestbook`, manual sync)을 추가했다.
...
## Autoresearch-airflow GitHub Actions GAR push (#121)

Autoresearch-airflow 저장소의 GitHub Actions가 SA key 없이 WIF로 GCP에
인증해 Artifact Registry `autoresearch-dev-docker`에 이미지를 push하는
경로다(`github_actions.tf`). 위 Cloud Build 경로(#32)와 현재 병존하며, GitHub
Actions 경로 end-to-end 검증 후 Cloud Build 경로 정리 여부를 별도 이슈로
결정한다(PR #128 코멘트 참조).

| 항목 | 값 | 비고 |
|---|---|---|
| SA | `autoresearch-dev-gar-pusher@ar-infra-501607.iam.gserviceaccount.com` | push 전용, 다른 권한 없음 |
| WI 가장 허용 | principalSet `attribute.repository/SKYAHO/Autoresearch-airflow` | 해당 리포 Actions만 이 SA 가장 가능 |
| Artifact Registry 권한 | `roles/artifactregistry.writer` on `autoresearch-dev-docker` | repository 단위(프로젝트 수준 아님) |

전제: bootstrap WIF provider의 `attribute_condition`이 Autoresearch-airflow
리포의 토큰 발급을 허용해야 한다(`allowed_github_repositories`,
[TERRAFORM_BOOTSTRAP.md](TERRAFORM_BOOTSTRAP.md)의 회귀 footgun 주의 참조).
`terraform-ci` SA 가장은 여전히 infra 리포만 가능하다(2단 경계).

Autoresearch-airflow 저장소 workflow에 필요한 값:

| 항목 | 값 |
|---|---|
| `workload_identity_provider` | `projects/<N>/locations/global/workloadIdentityPools/autoresearch-github/providers/github` |
...
- **제한**: WIF `attribute_condition` 은 허용 리포 목록(`allowed_github_repositories` — 현재 infra + Autoresearch-airflow, #121) 기반이지만, CI SA(`terraform-ci`) 가장 바인딩은 infra 저장소만 허용한다. workflow job guard로 fork PR이 아닌 내부 브랜치 PR에서만 plan을 실행한다.
- **apply 자동화는 범위 밖**(별도 이슈). 본 워크플로는 plan 만 게시한다.
- **drift 감지(#153)**: `.github/workflows/terraform-drift.yml`이 매일 09:23 KST에 dev root `plan -detailed-exitcode`를 실행하고, drift/오류 시 `[DRIFT]` 이슈를 생성(중복 시 코멘트)한다. CI SA viewer 권한만 사용 — apply 권한 없음. admin root는 master 접근 불가로 대상 외이며 운영자 로컬 plan으로 확인한다.
