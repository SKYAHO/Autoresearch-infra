...
> `roles/bigquery.jobUser`는 query/load job 실행에 필요하지만 project-level job 실행 권한이다. dev app/배치는 쿼리 실행 시 `maximum_bytes_billed` 같은 job-level 비용 제한을 함께 설정해야 하며, infra 차원의 쿼터 가드는 아래 "쿼리 비용 가드" 절을 따른다.

### 쿼리 비용 가드 (#22)

on-demand 쿼리는 스캔량 기준 과금($6.25/TiB)이라 잘못된 쿼리 하나가 비용을 튀게 할 수 있다. dev는 2중 가드를 사용한다.

| 계층 | 수단 | 값 | 강제 주체 |
|---|---|---|---|
| 1. infra 하드 상한 | Service Usage 커스텀 쿼터 (`bigquery_quota.tf`) | project 200 GiB/일, user/SA 100 GiB/일 | Terraform (초과 시 쿼리 실패) |
| 2. job 수준 상한 | `maximum_bytes_billed` | 쿼리당 **10 GiB** (10737418240 bytes) 기준 | 앱/배치 코드 (`SKYAHO/Autoresearch`) |

- **커스텀 쿼터**: `google_service_usage_consumer_quota_override`(google-beta)로 `bigquery.googleapis.com/quota/query/usage`의 `/d/project`, `/d/project/user` limit을 override한다. 단위는 **MiB**(콘솔 표시는 TiB): 204800 MiB = 200 GiB. 값은 `bq_query_usage_per_day_mib`, `bq_query_usage_per_user_per_day_mib` 변수로 조정한다.
- **최악 비용**: 200 GiB/일 상한 기준 ~$1.2/일(~$37/월). 배치 규모가 커지면 tfvars에서 상향한다.
- **`maximum_bytes_billed` 기준값**: dev 쿼리는 10 GiB/쿼리를 기본으로 하고, 이를 넘는 정당한 쿼리(예: Feast materialization 대량 backfill)만 개별 상향한다. BigQuery Python client는 `QueryJobConfig(maximum_bytes_billed=...)`, bq CLI는 `--maximum_bytes_billed`로 설정한다. 초과 시 과금 없이 실패한다.
- **적용하지 않는 방식과 이유**:
  - Reservation/slot(Editions): 고정비가 발생해 dev 트래픽에는 on-demand보다 비싸다. 운영 전환 시 재검토.
  - Dataset 수준 스캔 제한: GCP에 해당 기능이 없다. dataset 단위 통제는 IAM(dataEditor 범위)으로만 다룬다.
- **주의(409)**: 같은 쿼터를 콘솔에서 이미 수동 설정한 경우 apply가 `ALREADY_EXISTS`(409)로 실패한다. 이때는 기존 override를 `terraform import`로 가져온 뒤 apply한다.
- **롤백**: 리소스 삭제(또는 값 상향) 후 apply하면 기본 무제한으로 돌아간다. 데이터에는 영향 없다.
