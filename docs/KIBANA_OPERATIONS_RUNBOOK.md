...
`authenticated-emails` 파일이 **유일한** 이메일 경계다 — `--email-domain=*` 우회는
infra [#488](https://github.com/SKYAHO/Autoresearch-infra/issues/488)에서 제거됐고,
목록 밖 Google 계정은 proxy에서 403으로 거부된다.
**Kibana `/login`에서 다시 `elastic`(또는 별도
