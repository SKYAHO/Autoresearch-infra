...
| `terraform/admin/argocd-k8s` | 별도 state | ArgoCD namespace와 Helm values 설치 기반을 관리한다. 실제 ArgoCD 설치는 후속 이슈에서 진행한다. |
...
| ArgoCD/MLflow/ELK 후속 구현 | GitOps 설계는 정리했고, 실제 설치와 앱 연결은 후속 이슈에서 진행 |
...
ArgoCD 설치 기반은 `terraform/admin/argocd-k8s`에서 `argocd` namespace와
`helm-values/argo-cd.values.yaml` scaffold를 관리한다. 실제 Helm release와
AppProject/Application은 후속 이슈에서 추가한다.
