...
| BigQuery | `autoresearch_dev_analytics`, `feast_offline_store`(피처 테이블 4종 IaC 소유 #281) | 분석 데이터셋과 Feast offline store |
| Vertex AI | BigQuery↔Vertex `CLOUD_RESOURCE` connection(#281) | BigQuery ML `ML.GENERATE_EMBEDDING`이 다국어 임베딩 모델 호출 |
| MLflow | `mlflow` ns — tracking server, 전용 Cloud SQL DB/user + Secret, artifact GCS bucket, OAuth2-proxy, 내부 ILB(#91~#95/#232/#244) | 실험 추적 UI. 내부 ILB + Google OAuth |
...
| IAM / WI | GKE node SA, app SA, Airflow SA, Airflow batch SA, proxy SA, CI SA, GAR pusher SA(#121), 코드 업로더 SA(#238), Vault SA(#132), Cloud Build 전용 build SA(#269/#272), Vertex connection service agent(#281) | 워크로드별 최소 권한과 Workload Identity |
...
| CI 자동화 | PR plan(#6) + **일일 drift 감지**(#153) | drift 시 [DRIFT] 이슈 자동 생성. apply는 수동 유지 |
...
| Control plane endpoint | DNS endpoint 기본, `master_authorized_networks = []` | 팀원 IP allowlist 없이 IAM 기반으로 kubeconfig를 받는다. IP 예비 경로는 비어 있음(#279에서 개인 동적 IP 제거). |
...
- BigQuery `feast_offline_store`는 Feast feature table 저장소다. 피처 테이블 4종
  (`user_static_feature`, `user_dynamic_feature`, `video_feature`,
  `user_category_similarity`)은 Terraform이 스키마를 소유해(`ignore_changes` 없음)
  Feast `FeatureView` 계약 위반을 plan에서 잡는다(#281). BigQuery↔Vertex AI
  `CLOUD_RESOURCE` connection이 `ML.GENERATE_EMBEDDING`으로 다국어 임베딩을 호출한다.
...
| BigQuery Feast | `feast_offline_store` | Feast offline feature table 저장소다. 피처 테이블 4종은 Terraform이 스키마 소유(#281). |
| Vertex AI connection | BigQuery↔Vertex `CLOUD_RESOURCE` | `ML.GENERATE_EMBEDDING`이 `text-multilingual-embedding-002`를 호출(#281). connection은 모델명 미포함. |
...
| MLflow UI | 내부 ILB(`mlflow.dev.autoresearch.internal`) + OAuth2-proxy Google 로그인(#244). port-forward 대체 경로 | `TEAM_OPERATIONS_RUNBOOK.md`, `MLFLOW_OPERATIONS_RUNBOOK.md` |
...
| #87-#90 | Argo Rollouts controller + canary 실측 + runbook | GitOps 파일럿의 점진 배포 기반 |
| #96-#103 | ELK 스택(ECK/ES/Kibana/Filebeat/ILM/snapshot/runbook) | 로그 검색·분석을 Cloud Logging과 병행 |
| #91-#95 | MLflow tracking server + 전용 Cloud SQL DB + artifact GCS | 실험 추적. #232 OAuth2-proxy 인증, #244 내부 ILB 노출 |
| #129 | Feast Online Store Redis Cluster(2 shard, PSC + IAM/TLS) | dev online store. 접근은 egress + dbConnectionUser + CA secret 3종(#264) |
| #132-#136/#177 | Vault(학습용) single-node + KMS auto-unseal + 위협 게이트 | 학습·검증용. 실 서비스 secret은 Secret Manager 유지 |
| #153 | 일일 drift 감지 자동화 | dev root plan non-empty 시 `[DRIFT]` 이슈 자동 생성. apply는 수동 |
| #155/#232 | 내부 UI에 Google OAuth(Grafana) / OAuth2-proxy(Airflow·MLflow) | 내부 UI를 인증 뒤에 둔다 |
| #183/#186 | monitoring·argo-rollouts를 ArgoCD Application으로 이관 | Helm lifecycle을 GitOps로 관리 |
| #269/#272 | Cloud Build 전용 build SA 분리(1단계) | 앱 이미지 build를 기본 compute SA에서 분리, 간접 GAR push |
| #273/#274 | Cloud SQL tier db-f1-micro → db-g1-small 상향 | dev 동시 연결 수요 확보 |
| #280/#281 | Feast 피처 테이블 4종 IaC 편입 + Vertex AI connection | 스키마를 Terraform이 소유해 Feast 계약을 plan에서 검증 |
| #279 | GKE `master_authorized_networks`를 비움(개인 동적 IP 제거) | DNS 엔드포인트가 기본 경로. 동적 IP는 drift 반복 원인 |
...
| `YOUTUBE_PROXY_URL` 주입 + Cloud Run ID token 호출 | infra 권한(batch GSA invoker)은 준비됐으나 앱/Airflow 측 호출 로직이 필요 |
| Cloud Build #269 2단계 — 기본 compute SA writer 회수 | build 전용 SA 분리(1단계, #272)는 완료. 기본 compute SA의 GAR writer 회수는 앱 PR 대기 |
...
> ELK 운영 정책(ILM #101·GCS snapshot #102·Kibana runbook #103)과 MLflow 내부 ILB
> 노출(#244)은 완료되어 백로그에서 제거했다.
...
관리한다(manual sync). Grafana UI는 `kubectl port-forward`로 `localhost:3000`에서
접근하며, dashboard 운영 절차는 `docs/GRAFANA_OPERATIONS_RUNBOOK.md`를 기준으로 한다.

ArgoCD는 `terraform/admin/argocd-k8s`에서 `argocd` namespace와 argo-cd Helm
release(`10.1.3`), AppProject(`autoresearch-dev`), Application(`monitoring` #183,
`argo-rollouts` #186)을 관리한다. UI는 ClusterIP + `kubectl port-forward` 내부
접근만 허용하며, 접속·초기 credential 절차는 해당 root의 README를 기준으로 한다.
