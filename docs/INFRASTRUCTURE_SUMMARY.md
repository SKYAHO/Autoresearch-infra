...
| Secret(학습) | Vault 운영 제외(#412) | 실 서비스 secret은 Secret Manager. Vault dev root 코드(`vault.tf`)와 `terraform/admin/vault-k8s` root는 #478에서 삭제 완료, 원격 state 정리(forget/destroy)는 승인 대기 |
| KMS | `autoresearch-dev-vault` key ring / `vault-unseal` crypto key (#132) | Vault workload는 없음. #478에서 `vault.tf`를 삭제하며 이 2개는 `removed` 블록(forget)으로, 나머지 Vault IAM 리소스는 destroy 대상으로 분리 — 승인 apply 대기 |
