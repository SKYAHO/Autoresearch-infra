...
| Kubernetes admin roots | `terraform/admin/gke-team-access`, `terraform/admin/airflow-k8s` |
...
| #78 | monitoring Kubernetes admin root 분리 | Prometheus/Grafana namespace와 Helm values를 dev root 밖에서 관리 |
...
| Prometheus/Grafana 실제 Helm 설치 | #78에서 namespace와 values 경로를 준비하고, #79~#81에서 설치·접근·운영을 진행 |
...
values 파일 위치를 관리한다. 실제 Helm chart 설치와 dashboard 운영은 후속 이슈에서
단계적으로 추가한다.
