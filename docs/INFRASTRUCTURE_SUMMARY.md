...
| Redis | `autoresearch-dev-redis` | Feast Online Store, private PSA + AUTH/TLS (#129, apply 대기) |
...
| `terraform/admin/autoresearch-k8s` | 별도 state | 일반 앱 namespace/KSA와 Redis 포함 최소 egress NetworkPolicy를 관리한다(#129, apply 대기). |
...
| App KSA/GSA | `autoresearch-app` -> `autoresearch-dev-app` | 일반 앱 pod가 DB password, Redis AUTH/CA, Cloud SQL, GCS/BigQuery 등 필요한 권한을 얻는 경로다(#129, apply 대기). |
...
| Memorystore for Redis Basic 1 GiB | 서울 리전 시간당 과금, apply 전 Pricing Calculator로 재확인 |
...
| ArgoCD Airflow Application 생성 | AppProject 경계 PR(#125)은 umbrella chart 제안(Autoresearch-airflow#17) 회신 대기 |
| ELK 운영 정책 마무리 | 수집(#100)까지 완료. ILM 7일(#101), GCS snapshot(#102), Kibana runbook(#103) 남음 |
| MLflow 후속 구현 | 미착수 — 별도 설계 필요 |
