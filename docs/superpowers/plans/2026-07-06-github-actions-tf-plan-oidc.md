...
> 상태: Done (PR #15 merged, apply 완료)

> **에이전트 작업자 안내:** 이 계획은 작업 단위로 추적합니다. 단계는 체크박스(`- [ ]`) 형식을 사용합니다.

**목표:** PR 오픈 시 GitHub Actions가 Terraform `fmt/validate/plan`을 자동 실행해 PR 댓글로 게시하고, GCP 인증은 SA key 없이 GitHub OIDC + Workload Identity Federation으로 처리한다.

**아키텍처:** `terraform/bootstrap/`(local state)에서 GCS state 버킷·WIF 풀/프로바이더·CI SA를 1회성 생성하고, `terraform/envs/dev/`(GCS backend)는 그 버킷을 state로 사용한다. GitHub Actions runner는 OIDC 토큰으로 WIF를 거쳐 CI SA를 가장해 plan만(viewer + state bucket 접근) 실행한다.

**기술 스택:** Terraform >= 1.6 (google provider), GitHub Actions (`google-github-actions/auth@v2`, `hashicorp/setup-terraform@v3`, `actions/github-script@v7`), GCP Workload Identity Federation.
...
### 작업 1: `terraform/bootstrap/` 스캐폴드 (versions.tf + variables.tf)

**파일:**
- 생성: `terraform/bootstrap/versions.tf`
- 생성: `terraform/bootstrap/variables.tf`

**Interfaces:**
- 산출물: variable `project_id`(string, 필수), `github_repository`(string, default `SKYAHO/Autoresearch-infra`), `region`(string, default `asia-northeast3`). provider google(project=var.project_id)

- [x] **단계 1: versions.tf 작성**
...
### 작업 2: `terraform/bootstrap/main.tf` — 리소스 전부

**파일:**
- 생성: `terraform/bootstrap/main.tf`

**Interfaces:**
- 입력: 작업 1 variables(project_id, github_repository, region)
- 산출물: `google_storage_bucket.tfstate`(name=`autoresearch-dev-tfstate`), `google_iam_workload_identity_pool.github`(name=full WIF pool), `google_iam_workload_identity_pool_provider.github`(name=full provider), `google_service_account.terraform_ci`(email), IAM members, WI binding

- [x] **단계 1: main.tf 작성(locals + state bucket)**
...
### 작업 3: `terraform/bootstrap/outputs.tf`

**파일:**
- 생성: `terraform/bootstrap/outputs.tf`

**Interfaces:**
- 산출물: outputs `tf_state_bucket_name`, `tf_state_bucket_self_link`, `wif_pool_name`(full), `wif_provider_name`(full), `ci_service_account_email`

- [x] **단계 1: outputs.tf 작성**
...
### 작업 4: `terraform/envs/dev/versions.tf` — backend 블록 추가

**파일:**
- 수정: `terraform/envs/dev/versions.tf`(terraform 블록 내 backend 추가)

**Interfaces:**
- 산출물: dev 루트가 GCS backend 사용(bucket=`autoresearch-dev-tfstate`, prefix=`dev/`)

- [x] **단계 1: terraform 블록에 backend 추가**
...
### 작업 5: `.github/workflows/terraform-plan.yml`

**파일:**
- 생성: `.github/workflows/terraform-plan.yml`

**Interfaces:**
- 입력: GitHub variables `WIF_PROVIDER_ID`, `CI_SA_EMAIL`(bootstrap apply 후 등록)
- 산출물: PR 오픈/갱신 시 plan 실행 + 결과를 PR 댓글로 게시

- [x] **단계 1: workflow 파일 작성**
...
### 작업 6: 문서 (TERRAFORM_BOOTSTRAP.md + TERRAFORM_DEV.md + README.md)

**파일:**
- 생성: `docs/TERRAFORM_BOOTSTRAP.md`
- 수정: `docs/TERRAFORM_DEV.md`(CI 섹션 추가)
- 수정: `README.md`(진행 단계 한 줄)

- [x] **단계 1: `docs/TERRAFORM_BOOTSTRAP.md` 작성**
...
- [x] **단계 2: `docs/TERRAFORM_DEV.md` 에 CI 섹션 추가**
...
```
예상 결과: fmt OK, init OK, validate Success.

- [x] **단계 2: dev fmt + validate**
...
```
예상 결과: fmt OK, init OK, validate Success. backend 블록이 추가되어도 `-backend=false` 이므로 GCS 미연결 에러 없음.

- [x] **단계 3: whitespace 점검**
...
### 작업 8: 부트스트랩 apply + GitHub variables + dev 마이그레이션 (사용자 실행, GCP/GitHub 원격 — 보류)

**사용자 확인 후 실행**. 작업 1-7 코드 머지/커밋과 무관하게, 실제 GCP/GitHub 변경이 수반되므로 별도 게이트.

- [x] **단계 1: bootstrap apply(GCP)**

```bash
terraform -chdir=terraform/bootstrap init
terraform -chdir=terraform/bootstrap apply -var="project_id=ar-infra-501607"
```
예상 결과: bucket, bucket IAM, WIF pool, provider, CI SA, viewer, WI binding 리소스 add. 에러 시 API/권한 확인.

- [x] **단계 2: outputs 회수 → GitHub variables 4개 등록**
...
- [x] **단계 3: dev backend 마이그레이션**

```bash
terraform -chdir=terraform/envs/dev init -migrate-state
```
예상 결과: local → GCS 로 즉시 마이그레이션(state 비어있음).
...
### 작업 9: 커밋 + push + Draft PR (사용자 확인 후)
...
본문 체크리스트: bootstrap fmt/validate✅, dev fmt/validate✅(GCS backend 포함), workflow YAML✅. **보류**: bootstrap apply(GCP), GitHub variables 등록, dev migrate, workflow 실동작 테스트(작업 8, 사용자 실행 후).

---

## 셀프 리뷰

- **Spec 커버리지**: 4.1 bootstrap(bucket/pool/provider/SA/IAM/WI) → 작업 2. 4.2 dev versions backend → 작업 4. 4.3 workflow → 작업 5. 4.4 variables(4개) → 작업 6 BOOTSTRAP.md + 작업 8. outputs → 작업 3. 검증(10) → 작업 7. 적용순서(12) → 작업 8. 롤백(11) → BOOTSTRAP.md. 산출물(14) 전부 커버. 완료조건(15) 매핑: fmt/validate/plan workflow(5), id-token:write(5), GCS backend(4), CI SA key 없이(2+8), variables 문서화(6).
- **Placeholder scan**: 본문/PR body `"..."` 는 실제 실행 시 채우는 템플릿 자리(사용자 확인 단계). 코드 자체 placeholder 없음.
- **타입 일관성**: `local.wif_pool_id`/`wif_provider_id`/`ci_sa_id`(작업 2) ↔ outputs(작업 3) 리소스 참조 일치. WIF pool `name`(full) 을 member principalSet 에 사용(Terraform docs 준수).
