...
| Secret Manager | Airflow API key secret metadata와 Airflow SA accessor 관리. payload는 Terraform 밖에서 주입 | 실제 secret 값을 state/plan에 남기지 않음. 기존 app SA 접근은 전용 Airflow SA 접근으로 축소 |
