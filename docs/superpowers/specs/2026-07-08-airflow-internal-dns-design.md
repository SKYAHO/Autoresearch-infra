...
| VIP | Terraform 예약 내부 IP (`SHARED_LOADBALANCER_VIP`) | Helm 재설치에는 IP 유지. 주소 리소스 재생성 시 숫자 IP 재배정 가능성이 있으므로 Helm은 `airflow_ilb_ip` output 사용 |
...
- 비용: private zone $0.20/월 + 예약 내부 IP/passthrough ILB 무과금 수준.
...
- 참고: 예약 IP `purpose=SHARED_LOADBALANCER_VIP`는 GKE 공식 문서의 ILB 예약 IP
  ...
  현재 구성은 `address` 인자를 지정하지 않아 최초 생성 시 GCP가 주소를 배정한다.
  따라서 DNS/Helm 값은 숫자 IP 하드코딩이 아니라 `terraform output airflow_ilb_ip`
  기준으로 맞춘다.
