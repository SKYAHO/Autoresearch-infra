...
| 항목 | 상태 |
|---|---|
| Terraform backend | GCS backend(`autoresearch-dev-tfstate`, prefix `dev/`) 적용 완료 |
| `.github/` 디렉토리 | **없음** (workflow/CODEOWNERS 전무, 처음 생성) |
| repo 가시성 | **PUBLIC** (GitHub Actions 무료 한도 영향 없음) |
| `#1~#5` 코드 | main에 머지됨(최신 `e5ea31a`) |
| GCP apply | dev stack apply 완료(2026-07-06), 최종 plan `No changes` |
| 프로젝트 | `ar-infra-501607` |

**핵심 제약**: CI 머신이 같은 state에 접근해야 하므로 **GCS 원격 backend**가 본 이슈의 전제다. GCS 버킷과 WIF/CI SA는 Terraform 자체로 관리하되, dev 루트 모듈과 **분리된 `terraform/bootstrap/`** 디렉토리에 둔다(부트스트랩은 1회성, local state 유지).
...
| `google_service_account.terraform_ci` | CI용 SA. `terraform-ci@ar-infra-501607.iam.gserviceaccount.com` |
...
| variable | 값 예시 | 출처 |
|---|---|---|
| `GCP_PROJECT_ID` | `ar-infra-501607` | 고정 |
...
| `CI_SA_EMAIL` | `terraform-ci@ar-infra-501607.iam.gserviceaccount.com` | bootstrap output |

> `<N>`은 `ar-infra-501607`의 프로젝트 번호. bootstrap apply 후 `terraform -chdir=terraform/bootstrap output`으로 회수.
...
6. terraform plan → diff 계산 (viewer + state bucket 권한)
7. plan 결과를 PR 댓글 게시
...
| CI SA (`terraform-ci`) | `roles/viewer` | `ar-infra-501607` project |
...
> **secretAccessor를 왜 주지 않나?** 현재 `terraform/envs/dev`에는 Secret payload를 읽는 `data "google_secret_manager_secret_version"`가 없다. PR HEAD의 Terraform 코드가 실제 GCP 자격증명으로 plan되고 그 출력이 댓글로 게시되므로, CI SA에 전역 Secret Accessor를 주면 PR diff가 Secret 평문을 읽어 출력으로 노출할 수 있다.

## 7. 보안 고려

- **keyless**: SA key 파일을 만들지 않는다. key 유출/회전 부담 제거.
- **OIDC 토큰 수명 단축**: 잡 종료 시 만료. 저장/재사용 불가.
- **repo 한정 + fork 차단**: `attribute_condition`으로 다른 저장소 인증을 차단하고, workflow의 `github.event.pull_request.head.repo.full_name == github.repository` guard로 fork PR의 Terraform plan 인증을 막는다.
- **최소권한**: viewer + state bucket 접근. apply 권한과 Secret payload 접근 권한 없음.
...
| 임의 PR(포크 등) 인증 시도 | workflow job guard가 fork PR에서 plan job을 skip | WIF repo 조건만으로 포크 여부를 판단하지 않음 |
