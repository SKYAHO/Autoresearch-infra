...
| ES 구성 | **single-node** (master+data 겸용), heap 1GB / 컨테이너 memory request 2Gi·limit 3Gi, PVC 30Gi | dev 최소 비용. 실측: dev-default 단일 노드 여유 ~8.8GB(단일 노드 기준, 합계 아님)로 수용 가능 — **신규 node pool 불필요** |
...
| snapshot | GCS 전용 bucket, 일 1회, 7일 보관 | 정리는 **SLM retention**(bucket age lifecycle은 증분 세그먼트 공유를 깨뜨려 금지 — #102에서 정정). GSA는 bucket 단위 objectAdmin + legacyBucketReader |
| 복구 창 성격 | **디스크/노드 유실 시 최근 데이터 복구 전용** | ILM 7일 + snapshot 7일이라 실효 복구 창이 짧다(의도된 값). **장기 보관·감사 목적이 아니다** — 그 요구가 생기면 snapshot 보관만 별도로 늘린다(#102) |
...
| #98 ES 최소 클러스터 | `Elasticsearch` CR single-node, heap 1G/mem 2-3Gi, PVC 30Gi, TLS/auth 기본 유지. **index 기본 `number_of_replicas: 0`**(single-node에서 1이면 replica 배치 불가로 영구 yellow — green 기준 운영을 위해 필수). 배치 노드 headroom: request 2Gi 예약 후에도 해당 노드 여유가 3Gi 이상 남는지 확인(공용 pool burst 대비). CR은 kubernetes_manifest — CRD 부트스트랩 순서 주의(argocd 선례: operator 먼저 targeted apply) |
