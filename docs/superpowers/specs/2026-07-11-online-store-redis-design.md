...
- zone distribution: `SINGLE_ZONE` (`var.zone`, 기본 `asia-northeast3-a`)

`REDIS_SHARED_CORE_NANO`는 개발·테스트 전용이며 SLA가 없다. replica 0 구성은
노드 장애 시 가용성과 데이터 복구를 보장하지 않는다. dev GKE가 zonal cluster이므로
Redis primary shard 두 개도 같은 zone에 배치한다. 선택한 zone 장애 시 cluster 전체가
중단되고 데이터가 flush될 수 있다. Online Store 데이터는 offline store에서 다시
materialize할 수 있다는 전제로 persistence를 끄고 최소 비용으로 시작한다. 운영
전환 시 `REDIS_STANDARD_SMALL` 이상, `MULTI_ZONE`, `replica_count = 1`,
persistence를 함께 검토한다.
