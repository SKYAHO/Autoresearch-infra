...
- **plan/apply 노출(#211, 저장소 PUBLIC)**: terraform plan/apply 원문(허용 이메일 등
  속성값)을 공개 로그·artifact에 노출하지 않는다. (1) plan/apply 출력을 파일로
  리다이렉트하고 요약(리소스 헤더/`Plan:` 라인)만 STEP_SUMMARY에, (2) plan 바이너리는
  공개 artifact가 아니라 **private state 버킷**(`gs://autoresearch-dev-tfstate/admin-apply-plans/`)
  으로 job 간 전달(apply 후 삭제), (3) apply 오류 출력은 이메일 패턴을 마스킹.
- **admin 빈 값 방어(#305)**: admin 이메일 Secret 미설정/빈 값이면 `TF_VAR`가 빈
  문자열이 되어 terraform이 halt("Missing expression")한다(readonly와 달리 `[]`
  폴백 없음 — 의도된 안전장치). plan job에도 명시적 guard 스텝으로 먼저 실패시킨다.
- **workflow_dispatch 브랜치 방어**: WIF binding이 `workflow_ref @refs/heads/main`
  이라, feature 브랜치에서 dispatch하면 claim이 `@refs/heads/<branch>`가 되어 불일치
  → SA 가장 자체가 거부된다. SA의 넓은 권한(container.admin)은 main의 PR 리뷰를
  거친 이 workflow의 `root` options 범위 안에서만 노출된다(root 추가는 main PR 필요).
