# Airflow GKE Runtime Drift Design Spec
...
| `dev-default` node pool machine `e2-small -> e2-standard-4` | 코드화 | live resize가 적용되어 있고 system/GMP pod가 기본 pool에 배치되어 다음 apply의 downsize를 방지 |
