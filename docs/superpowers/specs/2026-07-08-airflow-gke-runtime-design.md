...
| GSA `autoresearch-dev-app` WI member `airflow/autoresearch-batch` | 후속 #62에서 전용 batch GSA로 대체 | Airflow KPO batch pod를 app GSA에서 분리해 최소 권한으로 축소 |
...
초기 drift 반영 시점에는 기존 app GSA(`autoresearch-dev-app`)가 raw GCS,
BigQuery, Feast 권한을 가지고 있었고, Airflow KPO batch pod가 같은 권한을
사용해야 해서 app GSA IAM binding에 아래 member를 추가했다.

```text
serviceAccount:ar-infra-501607.svc.id.goog[airflow/autoresearch-batch]
```

후속 #62에서 이 과도기 binding은 제거하고, batch 전용 GSA
(`autoresearch-dev-airflow-batch`)와 동일 KSA principal의 Workload Identity
binding으로 대체한다. batch GSA는 YouTube/OpenRouter secret accessor,
raw_data read/create, Feast registry/staging objectAdmin,
`feast_offline_store` dataEditor, project `bigquery.jobUser`만 가진다.
...
- Airflow SA와 batch SA의 각 secret `roles/secretmanager.secretAccessor`

app GSA의 Airflow API key accessor는 #62에서 제거한다. app GSA는 app KSA의
Cloud SQL/DB password 용도로만 유지한다.
