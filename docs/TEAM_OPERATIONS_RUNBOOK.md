...
| `roles/cloudbuild.builds.editor` + staging bucket objectAdmin | 프로젝트 / `<project_id>_cloudbuild` 버킷 | `gcloud builds submit`으로 이미지 빌드(#266). 버킷 권한은 source 업로드용. build는 기본 compute SA(dev GAR writer)로 실행되므로 빌드를 통한 간접 이미지 push가 가능하다는 점을 유의 |
