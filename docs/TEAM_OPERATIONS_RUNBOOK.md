...
4. 실제 이메일은 로컬 `terraform.tfvars`에만 넣고 커밋하지 않는다.
...
팀원은 클러스터 전체를 조회할 수 있지만, 변경 권한은 `airflow` namespace 내부로
제한된다. 새 namespace 생성, CRD 설치, ClusterRole/ClusterRoleBinding 생성, node
수정, 다른 namespace 작업은 허용하지 않는다.
