...
4. Grafana 접근이 필요한 팀원은 `terraform/admin/monitoring-k8s`의
   `grafana_viewer_user_emails`에 포함되어 있다.
...
| `monitoring` namespace Grafana port-forward | Kubernetes namespace | Grafana UI 접속을 위한 service/pod 조회와 port-forward |
...
| Grafana UI port-forward 실패 | monitoring namespace RBAC 미부여 또는 로컬 3000 포트 충돌 | `grafana_viewer_user_emails` 반영 여부 확인, 다른 포트 사용 |
