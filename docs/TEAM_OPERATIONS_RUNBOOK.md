...
|---|---|---|---|
| 박주용 | `roles/container.viewer`, `roles/iap.tunnelResourceAccessor`, `roles/compute.osLogin`, `roles/compute.viewer` | `airflow` namespace admin | GKE 조회/접속, Bastion 터널, Airflow 설치/운영 |
| 성효창 | `roles/container.viewer`, `roles/iap.tunnelResourceAccessor`, `roles/compute.osLogin`, `roles/compute.viewer` | `airflow` namespace admin | GKE 조회/접속, Bastion 터널, Airflow 설치/운영 |
| 이영준 | `roles/container.viewer`, `roles/iap.tunnelResourceAccessor`, `roles/compute.osLogin`, `roles/compute.viewer` | `airflow` namespace admin | GKE 조회/접속, Bastion 터널, Airflow 설치/운영 |
| 유현서 | `roles/container.viewer`, `roles/iap.tunnelResourceAccessor`, `roles/compute.osLogin`, `roles/compute.viewer` | `airflow` namespace admin | GKE 조회/접속, Bastion 터널, Airflow 설치/운영 |
| 최현규 | `roles/container.viewer`, `roles/iap.tunnelResourceAccessor`, `roles/compute.osLogin`, `roles/compute.viewer` | `airflow` namespace admin | GKE 조회/접속, Bastion 터널, Airflow 설치/운영 |
...
| Online Store Redis | `autoresearch` pod에서 private TLS endpoint 접속 | AUTH/CA는 app GSA로 Secret Manager에서 조회, 로컬 직접 접속 금지 (#129, apply 대기) |
...
|---|---|---|
| `autoresearch/autoresearch-app` | `autoresearch-dev-app@ar-infra-501607.iam.gserviceaccount.com` | 앱 DB와 Online Store Redis AUTH/CA secret 접근 (#129, apply 대기) |
