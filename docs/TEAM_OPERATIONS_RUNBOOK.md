...
| `roles/cloudbuild.builds.editor` + staging bucket objectAdmin | 프로젝트 / `<project>_cloudbuild` 버킷 | `gcloud builds submit`으로 이미지 빌드(#266). 버킷 권한은 source 업로드용 |
| `roles/cloudsql.viewer` | 프로젝트 | Cloud SQL 인스턴스 상태·private IP 조회(#266). DB 접속·데이터 권한은 아님 |
| `roles/secretmanager.secretAccessor` | `autoresearch-dev-db-password` secret만 | Airflow metadata DB 비밀번호 조회(#266). 프로젝트 수준 Secret Manager 권한은 부여하지 않음 |
