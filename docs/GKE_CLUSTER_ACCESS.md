...
터널 + 내부 ILB/DNS(#48)** 조합이다: SOCKS 프록시(`-D 1080`, 원격 DNS 조회)를 연
상태에서 `http://airflow.dev.autoresearch.internal:8080` 접속. 상세 명령은
`docs/TERRAFORM_DEV.md`의 #47/#48 섹션 참조.
...
# SOCKS 프록시: 내부 DNS 이름을 브라우저에서 그대로 사용
gcloud compute ssh autoresearch-dev-bastion \
  --zone asia-northeast3-a --project ar-infra-501607 --tunnel-through-iap \
  -- -N -D 1080
```

상세 스펙·비용·추가 사용법은 `docs/TERRAFORM_DEV.md`의 Bastion(#47) 섹션을 참조한다.
