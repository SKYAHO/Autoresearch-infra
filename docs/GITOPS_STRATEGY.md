...
| agent-orchestration (#453) | ✅ 완료(운영 중) | `deploy/agent-orchestration` 매니페스트 + `autoresearch-k8s` root(ns/KSA/NP). #526에서 고정 SHA·수동 sync를 없애고 `main` 추적 automated sync(prune·selfHeal 없음)로 전환, enabled=true(라이브 확인). 운영 절차 상세는 `docs/runbooks/2026-07-30-agent-orchestration-gke.md` — 이 파일은 #526 이전 고정 SHA 절차 기준이라 갱신 필요(#572). |
