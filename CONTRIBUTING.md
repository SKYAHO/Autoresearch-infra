...
1. **이슈 등록**: 코드가 변경되는 작업은 반드시 이슈를 먼저 만듭니다.
...
git fetch origin
git switch feat/42-add-cloud-run-job
```
...
- 에이전트 작업 규칙은 `CLAUDE.md`/`AGENTS.md`(symlink)와 `.claude/docs/`를 기준으로 합니다.
...
- 에이전트 가이드: `CLAUDE.md`, `AGENTS.md`(symlink), `.claude/docs/`
